...
function base_URL() {
	...
	// Работа с сайтами с использованием подкаталога
...
function validate_email($assoc, $key, $name = NULL) {
	...
	// Определите, действительна ли электронная почта
...
function validate_url($assoc, $key, $name = NULL) {
	...
	// Определите, действителен ли URL
	if(isset($assoc[$key]) && !empty($assoc[$key])) {
...
function validate_max_word_length($assoc, $key, $name = NULL) {
	...
	// Определить, содержит ли слишком длинное слово
...
function confirm_install() {
	...
	// Подтверждение того, что пароль по умолчанию был изменен
